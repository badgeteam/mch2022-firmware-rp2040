...
static bool is_error(uint32_t status) { return status == RSP_ERR; }

static uint32_t handle_sync(uint32_t *args_in, uint8_t *data_in, uint32_t *resp_args_out, uint8_t *resp_data_out) { return RSP_SYNC; }
...
static bool should_stay_in_bootloader() {
    bool wd_says_so = (watchdog_hw->scratch[5] == BOOTLOADER_ENTRY_MAGIC) && (watchdog_hw->scratch[6] == ~BOOTLOADER_ENTRY_MAGIC);
...
int main(void) {
    ...
    gpio_set_dir(FPGA_RESET_PIN, GPIO_OUT);
